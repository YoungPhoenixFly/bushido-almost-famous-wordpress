...
(function (wp) {
  ...
  const AfPoller = function () {
    ...
    function register(panelName, path, type) {
      const interval = type === 'archived' ? ARCHIVED_INTERVAL : ACTIVE_INTERVAL;
      endpoints[panelName] = {
        path,
        etag: '',
        interval,
    ...
    function fetchEndpoint(panelName) {
      ...
      wp.apiFetch({
        ...
        if (!response.ok) {
          if (onErrorCallback) {
            onErrorCallback(panelName, new Error('HTTP ' + response.status));
          }
          return;
        }
        ...
        return response.json().then(function (data) {
          const hasChanged = JSON.stringify(data) !== JSON.stringify(ep.lastData);
          ep.lastData = data;
    ...
    function setIntervalType(panelName, type) {
      ...
      const newInterval = type === 'archived' ? ARCHIVED_INTERVAL : ACTIVE_INTERVAL;
  ...
  const AfCampaignForm = function () {
    ...
    function updateReview() {
      ...
      const checked = form.querySelectorAll('[name="platforms[]"]:checked');
      ...
      setText('af-review-dates', startVal && endVal ? startVal + ' \u2013 ' + endVal : startVal || endVal);
      const countriesEl = form.querySelector('[name="targeting[countries]"]');
      const interestsEl = form.querySelector('[name="targeting[interests]"]');
      const parts = [];
      ...
      setText('af-review-total-budget', totalEl && totalEl.value ? '$' + parseFloat(totalEl.value).toFixed(2) : '');
      setText('af-review-daily-budget', dailyEl && dailyEl.value ? '$' + parseFloat(dailyEl.value).toFixed(2) : '');
      const modeEl = form.querySelector('[name="budget_allocation[mode]"]:checked');
      setText('af-review-allocation', modeEl ? modeEl.value.charAt(0).toUpperCase() + modeEl.value.slice(1) + ' mode' : '');
      const creativeIdsEl = document.getElementById('af-creative-ids');
      setText('af-review-creatives', creativeIdsEl && creativeIdsEl.value ? creativeIdsEl.value.split(',').length + ' selected' : 'None');
    }
    function bindSubmit() {
    ...
    function bindSaveDraft() {
      const draftBtn = document.getElementById('af-campaign-save-draft');
      if (!draftBtn) {
        return;
      }
    ...
    function setSubmitting(busy) {
      submitting = busy;
      const buttons = form.querySelectorAll('#af-campaign-submit, #af-campaign-save-draft');
    ...
    function loadCreativeOptions() {
      const container = document.getElementById('af-campaign-creatives');
      const hiddenInput = document.getElementById('af-creative-ids');
      if (!container || !hiddenInput) {
        return;
      }
      let selected = hiddenInput.value ? hiddenInput.value.split(',').map(function (id) {
        return id.trim();
      }).filter(Boolean) : [];
      wp.apiFetch({
        ...
        if (!Array.isArray(creatives) || !creatives.length) {
          replaceTextState(container, 'af-empty-state', afI18n('noCreatives', 'No approved creative assets are available yet.'));
          return;
        }
        ...
        creatives.forEach(function (creative) {
          ...
          checkbox.checked = selected.indexOf(creative.id) !== -1;
          const title = document.createElement('strong');
          title.textContent = creative.name || '(Untitled)';
          const meta = document.createElement('span');
          meta.className = 'description';
          meta.textContent = String(creative.approved_count || 0) + ' / ' + String(creative.total_formats || 0) + ' approved';
          card.appendChild(checkbox);
          card.appendChild(title);
          card.appendChild(meta);
          container.appendChild(card);
          checkbox.addEventListener('change', function () {
            selected = Array.from(container.querySelectorAll('input[type="checkbox"]:checked')).map(function (input) {
              return input.value;
            });
        ...
        replaceTextState(container, 'af-empty-state', afI18n('creativeLoadFail', 'Unable to load creative assets right now.'));
      });
    }
    ...
    function setNestedValue(obj, key, value) {
      ...
      for (let i = 0; i < parts.length - 1; i++) {
        const part = parts[i];
        if (!current[part]) {
          current[part] = parts[i + 1] && !isNaN(parts[i + 1]) ? [] : {};
        }
  ...
  const AfBudgetAllocation = function () {
    ...
    function bindModeToggle() {
      const radios = document.querySelectorAll('[name="budget_allocation[mode]"]');
      ...
      const checked = document.querySelector('[name="budget_allocation[mode]"]:checked');
      if (checked) {
        mode = checked.value;
      }
    }
    ...
    function bindSliders() {
      table.querySelectorAll('.af-allocation-slider').forEach(function (slider) {
        slider.addEventListener('input', function () {
          const row = slider.closest('.af-allocation-row');
          const pctInput = row.querySelector('.af-allocation-pct-input');
          if (pctInput) {
    ...
    function bindInputs() {
      table.querySelectorAll('.af-allocation-pct-input').forEach(function (input) {
        input.addEventListener('input', function () {
          const row = input.closest('.af-allocation-row');
          const slider = row.querySelector('.af-allocation-slider');
          if (slider) {
      ...
      table.querySelectorAll('.af-allocation-fixed-input').forEach(function (input) {
        input.addEventListener('input', function () {
          recalculate();
        });
      });
    ...
    function recalculate() {
      ...
      const dailyInput = document.getElementById('af-daily-budget');
      const totalBudget = dailyInput ? parseFloat(dailyInput.value) || 0 : 0;
      if (mode === 'percentage') {
        let pctTotal = 0;
        rows.forEach(function (row) {
          ...
          pctTotal += pct;
          const preview = row.querySelector('.af-alloc-preview-amount');
          if (preview) {
            preview.textContent = (totalBudget * pct / 100).toFixed(2);
          }
        });
        ...
        if (errorEl) {
          errorEl.style.display = Math.abs(pctTotal - 100) > 0.01 ? '' : 'none';
        }
      } else {
        let fixedTotal = 0;
        rows.forEach(function (row) {
          const fixedInput = row.querySelector('.af-allocation-fixed-input');
          const amount = fixedInput ? parseFloat(fixedInput.value) || 0 : 0;
          fixedTotal += amount;
          const preview = row.querySelector('.af-alloc-preview-amount');
          if (preview) {
        ...
        const totalEl2 = document.getElementById('af-alloc-fixed-total');
        const errorEl2 = document.getElementById('af-alloc-fixed-error');
        if (totalEl2) {
          totalEl2.textContent = fixedTotal.toFixed(2);
        }
        if (errorEl2) {
          errorEl2.style.display = fixedTotal > totalBudget ? '' : 'none';
        }
      }
    }
    return {
  ...
  const AfAudienceBuilder = function () {
    ...
    function bindDelete() {
      document.querySelectorAll('.af-audience-delete').forEach(function (btn) {
        btn.addEventListener('click', function () {
          ...
          const confirmed = window.confirm(afI18n('deleteAudience', 'Delete this audience?'));
          if (!confirmed) {
            return;
          }
          btn.disabled = true;
          wp.apiFetch({
            ...
            // eslint-disable-next-line no-alert
            window.alert(afI18n('deleteAudienceErr', 'Could not delete the audience. Please try again.'));
          });
        });
      });
    }
    ...
    function bindPlatformConfig() {
      ...
      const toggle = function () {
        ...
        form.querySelectorAll('.af-audience-google-only').forEach(function (row) {
          row.hidden = !isGoogle;
        });
      };
    ...
    function buildCreatePayload() {
      ...
      const platformSelect = document.getElementById('af-audience-platform');
      ...
      if (platform === 'google') {
        const config = {};
        const description = form.querySelector('[name="config_description"]');
        const lifespan = form.querySelector('[name="config_membership_life_span"]');
    ...
    function bindSubmit() {
      form.addEventListener('submit', function (e) {
        ...
        const notices = document.getElementById('af-audience-notices');
        notices.replaceChildren();
        const data = buildCreatePayload();
        const endpoint = form.dataset.apiEndpoint;
        const method = form.dataset.apiMethod;
        if (!hasCredentialFor(data.platform)) {
          replaceTextNotice(notices, 'error', afI18n('noPlatformCredential', 'No connected credential for the selected platform. Connect one on the Accounts page first.'));
          return;
        }
        setSubmitting(true);
        wp.apiFetch({
    ...
    function bindLookalike() {
      ...
      const lookalikeForm = document.getElementById('af-lookalike-form');
      document.querySelectorAll('.af-audience-lookalike').forEach(function (btn) {
        btn.addEventListener('click', function () {
          ...
          const sourceIdInput = document.getElementById('af-lookalike-source-id');
          const sourceNameEl = document.getElementById('af-lookalike-source-name');
          const nameInput = document.getElementById('af-lookalike-name');
          if (sourceIdInput) {
          ...
          if (nameInput) {
            nameInput.value = afI18n('lookalikeNameFmt', 'Lookalike — %s').replace('%s', sourceName);
          }
          panel.hidden = false;
      ...
      lookalikeForm.addEventListener('submit', function (e) {
        ...
        const notices = document.getElementById('af-lookalike-notices');
        notices.replaceChildren();
        const sourceIdInput = document.getElementById('af-lookalike-source-id');
        ...
        const config = {};
        const nameInput = document.getElementById('af-lookalike-name');
        const ratioInput = document.getElementById('af-lookalike-ratio');
        const countryInput = document.getElementById('af-lookalike-country');
        ...
        wp.apiFetch({
          path: '/almost-famous/v1/audiences/' + sourceId + '/lookalike',
          method: 'POST',
          data: {
            config
          }
    ...
    function bindLookalikeRatio() {
      ...
      const display = document.getElementById('af-lookalike-ratio-value');
      if (!slider || !display) {
        return;
      }
      slider.addEventListener('input', function () {
    ...
    function setLookalikeSubmitting(busy) {
      ...
      if (busy) {
        submitBtn.dataset.afLabel = submitBtn.textContent;
        submitBtn.textContent = afI18n('creatingLookalike', 'Creating…');
      } else if (submitBtn.dataset.afLabel) {
        submitBtn.textContent = submitBtn.dataset.afLabel;
        delete submitBtn.dataset.afLabel;
      }
  ...
  const AfCreativeManager = function () {
    ...
    function bindMediaButtons() {
      bindMediaButton('af-select-media', 'af-source-attachment-id', 'af-selected-media-name');
    }
    function bindMediaButton(buttonId, inputId, nameId) {
      ...
      button.addEventListener('click', function () {
        ...
        frame.on('select', function () {
          ...
          if (nameDisplay) {
            nameDisplay.textContent = attachment.filename || attachment.title;
          }
        });
        frame.open();
      });
    ...
    function bindDetailViewPolling() {
      ...
      const interval = parseInt(statusEl.dataset.pollInterval, 10) || 5000;
      if (!creativeId) {
        return;
      }
      ...
      const poll = function () {
        ...
        wp.apiFetch({
          ...
          const progress = document.getElementById('af-processing-progress');
          if (progress && payload.progress) {
            progress.textContent = afI18n('progressFmt', 'Progress: %s%').replace('%s', payload.progress);
          }
    ...
    function renderStillProcessingNotice(statusEl) {
      ...
      p.textContent = afI18n('processingTimeout', 'Still processing. Refresh the page later to check on progress.');
    ...
    function renderFailureNotice(statusEl) {
      ...
      const p = document.createElement('p');
      p.textContent = afI18n('assetProcessingFailed', 'Asset processing failed. Please try again.');
      notice.appendChild(p);
      statusEl.appendChild(notice);
    }
    function bindListGridPolling() {
      const cards = document.querySelectorAll('.af-creative-card[data-af-creative-status="processing"]');
      if (!cards.length) {
        return;
      }
    ...
    function pollCardUntilTerminal(card) {
      ...
      const poll = function () {
        ...
        wp.apiFetch({
          ...
          if (err && (err.status === 404 || err.code === 'rest_no_route')) {
            return;
          }
    ...
    function updateCard(card, status, payload) {
      ...
      if (status === 'complete' && payload && payload.thumbnail_url) {
        const thumb = card.querySelector('.af-creative-card__thumb img');
        if (thumb) {
